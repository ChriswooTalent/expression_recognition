...
class FER2013(data.Dataset):
    ...
    def __init__(self, split='Training', filename='./data/data.h5', train_length=28709, validate_length=3589, test_length=3589, resize_length=96, transform=None):
        ...
        if self.split == 'Training':
            ...
            self.PublicTest_data = self.PublicTest_data.reshape((validate_length, resize_length, resize_length))

        else:
            self.PrivateTest_data = self.data['PrivateTest_pixel']
            self.PrivateTest_labels = self.data['PrivateTest_label']
            self.PrivateTest_data = np.asarray(self.PrivateTest_data)
            self.PrivateTest_data = self.PrivateTest_data.reshape((test_length, resize_length, resize_length))
