...
dst_resizelength = 70
datapath = os.path.join('data', 'data_wild_3C.h5')
        ...
        if file_ext in img_ext:
            ...
            for key in expression_dict.keys():
                if subpath_aplit[-1] == key:
                    ...
                    I_color = img_swap1.reshape((1, 3 * dst_resizelength * dst_resizelength))
                    I = I_gray.reshape((-1))
                    I_c = I_color.reshape((-1))
                    expression_list[subpath_aplit[-1]].append(I_c.tolist())
                    break
